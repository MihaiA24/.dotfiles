...
### 6. Tuning plan — applied 2026-07-28, judged 2026-08-11

Standing recommendation: **OMP as primary, tuned.** The recommended defaults below were **applied on 2026-07-28** ("go with them"). The questions stay open — what is parked for the next grilling session is no longer *whether* but *whether it worked*, judged against the pre-registration at the end of this section.

**Q-A — What compaction threshold?** ✅ **Applied: `thresholdTokens = 150000`, `idleEnabled = true`** (top of the recommended 120–150 K band — conservative first step; backup at `~/.omp/agent/config.yml.bak-tuning`). Context previously accreted to the provider ceiling before compaction; cache reads are ~69% of cost and average context follows a sawtooth up to the threshold. Counterarguments stand: each compaction costs a summarization call plus cache re-priming at 1.25×; the −34% figure is arithmetic, not measurement. *Still to grill: the number itself, once measured.*

**Q-B — `compaction.strategy = handoff`?** ✅ **Applied: `strategy = handoff`, `handoffSaveToDisk = true` — the A/B treatment arm is now running**, judged against the historical baseline (22.8 M tokens/session, end-green 91.2%). This is the Hermes unit-boundary pattern: 30% of its sessions end on compression at 3.5 M tokens/session. Known confound, accepted: Q-A and Q-B change together, so tokens/session attribution is to the *bundle*; if the bundle regresses, `strategy` reverts first. What died on inspection: Hermes' kanban (every table 0 rows — do not port) and "OMP under-delegates" (false: 219 subagent session files).

**Q-C — What verification cadence?** ✅ **Applied: `omp/hooks/cadence-governor.ts` at N=25** (Hermes ~18, OMP ~37 — midpoint), wired via `extensions` beside the recorder; 8 tests pass. Nudges ride the documented `tool_result` content-override path — nothing new enters the transcript. Hook, not skill: prose mandates measured 15.6% adherence even repeated three times; Ponytail self-activated 0/10.[J3] *Still to grill: the constant, and whether nudges degrade long autonomous runs — the recorder now measures cadence directly.*

**Q-D — Mechanism for the write:edit fix?** ✅ **Applied, with one deviation from the recommendation, reasoned:** the "rule first" default died on inspection — a rule cannot test file existence, so it would fire on all `write` calls (5,170 — the most-called tool), injecting reminder text into every one. Implemented instead in the same governor hook: `write` results on pre-existing files ≥ 1 KiB get a one-line steer to `edit`. Targeted, zero transcript noise on new files. *Still to grill: does discouraging `write` push the agent into worse substitutes (fragment edits, heredoc piping that evades measurement)? Judged by the write:edit ratio, baseline 3.15.*

**Q-E — Formalize model routing?** ✅ **Already satisfied — no change needed.** `modelRoles.default` is `claude-opus-5`; the current session model is claude-family. The measured effect stands: Claude-family ~98% verification pass, GPT-family 74–85%, consistent across harnesses (decisive control: gpt-5.3-codex-spark *on OMP* = 73.5%). The discipline to keep: don't hand verification-critical sessions to GPT/volume models. *Still to grill: cost delta of Claude-heavy routing, and the never-random task→model confound.*

**Q-F — What happens to Hermes?** **(b) and (c) applied; (a) remains the user's call.** The blocking question was answered by reading the source: `memory.provider: agentmemory` is **dangling** — provider plugins load from `plugins/memory/` (byterover, hindsight, holographic, honcho, mem0, openviking, retaindb, supermemory) and `~/.hermes/plugins/`; no `agentmemory` plugin exists in either, and the 119 `memory`-tool calls run on the built-in file store. So all four dead MCP servers are now `enabled: false` (agentmemory 13 calls, mempalace 24, serena 36, codebase-memory-mcp 74, across 782 sessions; backup at `config.yaml.bak-tuning`), and `HERMES.md` is down from three lean-ctx blocks to the single richest informational one (backup at `HERMES.md.bak-tuning`). *(a) — keep for short-unit work, let fade, or decommission — is genuinely open; Hermes has been idle since 07-14.*

**Q-G — When is the data enough to judge?** **Pre-registered 2026-07-28, before any post-change data existed:**

| Metric | Baseline | Success at 2026-08-11 | Source |
|---|---:|---|---|
| Tokens/session (median, main sessions) | 22.8 M | **≤ 16 M** (−30%) | session jsonl |
| Sessions ending green | 91.2% (n=57) | **≥ 92%** on n ≥ 40 new | recorder |
| Verification cadence (calls between runs) | ~37 | **≤ 25** | recorder |
| write:edit ratio | 3.15 | **< 2.0** | session jsonl |
| Rollback trigger | — | end-green < 88% or the boundary demonstrably loses needed context → revert `strategy` first, then threshold | recorder + experience |

Judgment date **2026-08-11** (~2 weeks). Do not peek and adjust before then except on the rollback trigger — the JetBrains ladder exists because k=1 lied to them twice, and their smoke runs lied in *both directions*.
