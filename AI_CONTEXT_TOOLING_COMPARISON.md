...
| OMP extensions | `omp/hooks/verification-recorder.ts` + `omp/hooks/cadence-governor.ts` (N=25, silent after 3 unheeded nudges, write-steer ≥1 KiB) | `omp config set extensions '[]'` |
...
**Verdict: OMP primary (capability: Anthropic models, LSP, recoverable anchors; revealed preference), tuned via Hermes' two good ideas (bounded units, verification cadence). Not substitutes; definitive answer needs paired benchmark (§Open).** Others: opencode installed+configured — corrected 08-11: 285 db sessions, of which 278 (Jun) are benchmark residue under `code/nter/model-benchmarking…` (244 distinct dirs, one-shot pattern); interactive use ≈5 sessions ever, last 08-10 (capability candidate, zero comparative evidence); Codex 73 / Claude Code 5 sessions, dormant; cursor=IDE; qwen/factory/continue/goose empty. Dead on inspection: Hermes kanban (0 rows), "OMP under-delegates" (219 subagent files), Hermes lean-ctx "duplicate" (version rename).
...
### Round 2 — full-market deep sweep (2026-08-11)

One-off ordered deep-dive, not a revival of the retired trending channel: 3 librarians read primary sources (repo/source-file/registry reads, live GitHub snapshots) for 17 tools across token/graph/memory lanes, cross-checked against the live machine inventory (OMP 17.2.13 active · Hermes 0.20.0 dormant since 07-14 · Claude Code 4 sessions · opencode — §Harness verdict correction) and OMP's native coverage map (structural reads, selectors, artifact spill, pruning/shake/snapcompact, provider cache management, Mnemopi, LSP/AST, subagents, skills, MCP client, hooks — native coverage of every candidate lane except persistent code graph = the gated cbm slot). **Result: 0 adoptions, 0 grade changes. No candidate ships 3P agent-on-repo evidence; every number below is 1P.** Full reports: `history://{Token,Graph,Memory}ToolsLibrarian` (session artifacts).

New candidates (9):

| Candidate (ver · ★ 08-11) | 1P claim | Grade | Verdict |
|---|---|---|---|
| Context Mode 1.0.169 · 19.8K | 96–98% context bytes (21 own scenarios) | B | reject: duplicates OMP-native spill/selectors/compaction; sits on read→edit path (rule 3); ELv2 |
| mcp-compressor 0.31.7 · 106 | 70–97% of MCP schema tokens | C | reject: solves schema bloat this stack doesn't have — OMP `mcpServers: {}` |
| GitNexus 1.6.9 · 45.3K | code graph + PDG/taint; **no token bench** | D | reject: PolyForm-NC license; redundant with gated cbm; rerun-`analyze` staleness |
| GraphRAG · cognee · Graphiti · Semantica (35.4K/30.0K/29.8K/4.7K) | document/temporal/enterprise KGs; **no coding measurement in any** | D | reject: wrong lane — no code semantics; cognee/Graphiti also hit one-memory-owner (rule 5); GraphRAG has no MCP; Semantica's MCP graph is process-RAM unless exported |
| Letta 0.16.8 · 24.2K | self-editing memory blocks | D | reject: competing agent runtime, not a memory MCP; repo self-labeled legacy; blocks ride every prompt |
| Zep v3 (plugin 0.3.0) · 4.8K | 1,378 tok/query · 0.24 s (own committed LoCoMo run) | C | reject: paid cloud only; no public memory MCP; product source closed |

Already-tracked, re-verified — no grade moves: **Headroom** 0.34.0 (owner chopratejas→headroomlabs-ai; coding claim now 15–20%; P1's +48.4% stands; published latency 1 ms–2 s by payload, benchmarked on an older version) · **rtk** 0.45.0 (README still honest, telemetry still bytes/4) · **LLMLingua** 0.2.2 (last release 2024-04; default = 7B CUDA model) · **cbm** 0.10.1 · 38.6K (graph lane's librarian independently lands on "cbm as sole code-graph trial" = our existing gated slot; litmus 0 fires 08-05→08-11 — stays gated) · **agentmemory** 0.9.29 (92% = one example at 240 observations; 54-tool MCP surface; npm lags source) · **MemPalace** 3.7.0 (30× compression self-retracted: AAAK 84.2 vs raw 96.6 R@5) · **Hindsight** 0.9.0 (default recall cap 4,096 tok/call; PG+pgvector+reranker+LLM stack) · **mem0** 2.0.17 (**README states the 94.4% headline uses proprietary managed optimizations absent from the OSS SDK** — local arms must expect OSS ≠ headline).

Denominator trap: rtk counts bash bytes, Context Mode context bytes, mcp-compressor schema tokens, Headroom/LLMLingua prompt tokens, memory tools injected-context tokens — none measures success-adjusted billed cost (rule 1); the percentages are mutually incomparable and individually unaudited.

## Open

**Executed 2026-08-11 (both pre-registered gates):** read-out graded against the frozen table — bundle kept, 3/4 pass (§State) — and the lean-ctx kill threshold fired. Final semantic count **sem=1** since 07-28: one `ctx_search(action=semantic)` call (mimir, 08-10T18:32Z), landed after the 08-10 `sem=0` sweep of all 54 sessions; still < 5 with margin. Drop executed as pre-registered: `mcpServers.lean-ctx` removed from `~/.omp/agent/mcp.json` AND `"lean-ctx"` added to `disabledServers` (backup `mcp.json.bak-leanctx-drop`); `~/.claude.json` still registers lean-ctx + cbm and the denylist covers both, so the import cannot resurface them. Live omp instances started 08-10 (pre-edit) still hold lean-ctx children per the connect-time staleness rule — restart them; a fresh instance must show no lean-ctx child and no `ctx_search` in inventory. Hook live-load confirmed 08-10: `verification_events` 281 → 308. Round-3 re-grill executed same day (§below) — nothing time-gated remains. Post-drop prose converged: the `~/.claude/CLAUDE.md` scope line (marker `lean-ctx-claude-v6` kept) and the lean-ctx skill body (repo `skill_bodies/lean-ctx.md` + host `~/.claude/skills/lean-ctx/SKILL.md`, byte-identical) now state lean-ctx is not mounted on OMP at all — the lean-ctx skill still rides OMP rosters via the `~/.claude/skills` root (not in `OMP_SKILL_ROOTS`), so its body must not point at absent `ctx_*` tools; 39 unit tests green.

**Grilled 2026-08-11 — round 3 (Q-A number, Q-C constant), against the 60 sessions since 07-28:**
1. **Q-A `thresholdTokens=150000` — KEPT.** The trigger binds exactly as designed: 53 compactions across 24/60 sessions, `tokensBefore` median 150,988, only 3/53 overshoot >160K (max 214K — one long turn between checks); 36/60 sessions never reach the threshold. No paired arm exists for 120K-vs-150K and the bundle passed its frozen table, so moving the number now would be goalpost drift with zero counter-evidence. Compaction cost stays bounded at ~0.9 fires/session. Revisit rule unchanged: rollback on end-green < 88%.
2. **Q-C `N=25` — constant KEPT, misfire fixed.** First-fire compliance is real: 59% verify within 25 calls of the first nudge (76 first-fires → 31 second-fires). But ignored nudges never converge — continuation rises 41%→65%→70%+, 16/44 nudged sessions reached N≥100, 5 reached N≥400 (max 1600; benchmarking/teaching sessions with no classifiable test/lint) — **192/319 fired nudges (60%) were spam beyond N=75**. That is the hook's own pre-registered revisit condition ("revisit only if the recorder shows the nudges misfiring"). Fix is wiring, not prose: governor goes silent after 3 unheeded nudges (fires at 25/50/75, quiet until a verification re-arms it) — `cadence-governor.ts` + new test, 9 hook tests green; live instances pick it up on restart.
3. Flag, not a criterion: marginal per-command pass rate ≈73% over recorder events 282–374 (94.3% cumulative at n=281 → 88.6% at n=378). Premature nudge-induced verification or just harder work — recorder keeps measuring; look again at the next read-out.
...
**ADR-0006 flip trigger FIRED 2026-08-11:** decision recall failed 4/4 probes; plain-text ADRs carried the truth (§State). Next: local Hindsight-vs-mem0 bake-off, endpoints pre-registered before any adoption — same trigger discipline as ADR-0006. **Arms pinned by Round 2 (§Evidence):** Hindsight 0.9.0 self-hosted (PG/pgvector + cross-encoder; default recall 4,096 tok/call — cap it) vs mem0 OSS 2.0.17 (Qdrant+SQLite; 94.4% headline is managed-only, vendor-stated) vs Mnemopi incumbent as control. agentmemory enters only if cross-harness memory becomes a requirement (it isn't: OMP-primary, rule 5).
...
- [C1 — OmniRoute](https://github.com/diegosouzapw/OmniRoute) · [C2 — Headroom](https://github.com/headroomlabs-ai/headroom) · [C2B — benchmarks](https://docs.headroomlabs.ai/docs/benchmarks) · [C3 — Caveman honest numbers](https://github.com/JuliusBrussee/caveman/blob/main/docs/HONEST-NUMBERS.md) · [C4 — lean-ctx](https://github.com/yvgude/lean-ctx) · [C5 — Context Mode](https://github.com/mksglu/context-mode) · [C5B — benchmark](https://github.com/mksglu/context-mode/blob/main/BENCHMARK.md) · [C6 — mcp-compressor](https://github.com/atlassian-labs/mcp-compressor) · [C6B — Atlassian post](https://www.atlassian.com/blog/development/mcp-compression-preventing-tool-bloat-in-ai-agents) · [C7 — LLMLingua](https://github.com/microsoft/LLMLingua) · [C8 — rtk](https://github.com/rtk-ai/rtk)
- [M1 — agentmemory](https://github.com/rohitg00/agentmemory) · [M2 — MemPalace](https://github.com/MemPalace/mempalace) · [M2H — corrections](https://github.com/MemPalace/mempalace/blob/develop/docs/HISTORY.md) · [M3 — Hindsight](https://github.com/vectorize-io/hindsight) · [M3P — paper](https://arxiv.org/abs/2512.12818) · [X1 — mem0](https://github.com/mem0ai/mem0) · [M4 — Letta](https://github.com/letta-ai/letta) · [M5 — Zep](https://github.com/getzep/zep)
- [G1 — Graphify](https://github.com/Graphify-Labs/graphify) · [G2 — codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp) · [G2P — paper](https://arxiv.org/abs/2603.27277) · [G3 — GitNexus](https://github.com/abhigyanpatwari/GitNexus) · [G4 — GraphRAG](https://github.com/microsoft/graphrag) · [G5 — cognee](https://github.com/topoteretes/cognee) · [G6 — Graphiti](https://github.com/getzep/graphiti) · [G7 — Semantica](https://github.com/semantica-agi/semantica) · [X2 — Serena](https://github.com/oraios/serena) · [X3 — aider repo map](https://github.com/Aider-AI/aider/blob/main/aider/repomap.py)
