...
- **Memory: decided 2026-08-11 (user) — Mnemopi stays default; Hindsight kept as sole challenger; bake-off deferred.** Deep research (`agentic-env/docs/memory-backend-research.md`) narrowed arms: Hindsight is the only candidate with multi-project scoping (banks + tag unions `any_strict`) and has a native `memory.backend=hindsight` (OMP caps recall at 1,024 tok). Eliminated: mem0 OSS (no OMP backend, hosted-only MCP, 97.8%-junk report mem0#4573); agentmemory (recall not project-scoped as shipped; "95.2%" is retrieval-only R@5). No independent benchmark exists for ANY memory tool. When the bake-off runs: replay the 4 failed decision-recall probes + cross-project leakage probes, judge by billed cost per successful probe. Local check: dotfiles bank has 38 embeddings — arm64 bug omp#3054 is not our failure mode.
