...
- **Decided:** OMP, floor `18.1.14` (`stack_metadata.OMP_VERSION`). Contract settings were reviewed at `v18.1.14`. Installation follows GitHub's latest stable release redirect, downloads the platform binary directly, checks it against that release's `SHA256SUMS.txt`, verifies the downloaded executable's version floor and atomically replaces the installed binary. The checksum proves download integrity, not an independent publisher signature.
...
**Source drift:** `agentic-skill-drift` (read-only) compares each curated skill with its pack source, with its reviewed fingerprint in `agentic_env/skill-fingerprints.json`, and upstream at the pinned revision with upstream today. Its installed comparison covers the canonical managed store, not arbitrary standalone destinations. Vendored adaptations never count as drift, because the upstream comparison is upstream-to-upstream. It exits 1 on a moved tag, an unreviewed vendored edit, a missing or locally modified install, or a remote install that the skills CLI lockfile attributes to another pack. For vendored installs, matching reviewed bytes take precedence over stale lock origins. Packs still move only by reviewed tags or vendored updates. Re-record the baseline with `--update-baseline` as part of that review, never to silence a finding; scoped updates preserve all unselected pack entries and refuse an unreadable existing baseline.
...
- Clean-host contract (2026-09-02; version floors 2026-09-16): `uv tool install --force . && agentic-bootstrap` on a host with a `~/.dotfiles` checkout yields exactly this stack. Reviewed versions are **floors**, not pins (`STACK_VERSION_FLOORS`). Installers fetch the latest release and verify it is at or above the floor, so a host ahead of the review is compliant and a host below it is a doctor failure. Skill, MCP, and hook drift is never tolerated. Two components are still fetched at a fixed identity. Hermes installs release `29112bef` (`HERMES_COMMIT`) through an installer script fetched from commit `95d42656` (`HERMES_INSTALL_COMMIT`, #39). The installer refuses to roll a checkout backwards, so a host at or above the floor is left alone. The codebase-memory-mcp release archives are pinned by per-arch SHA256. Remote scripts stay hash-pinned regardless. The npm references install `@latest` and carry a written contract reason (`remote_install_contract`).
...
- Hermes installer (2026-09-15, #39): fetched at commit `95d42656` of `NousResearch/hermes-agent` (the bytes reviewed 2026-09-02). It is never fetched from the floating `hermes-agent.nousresearch.com/install.sh`, which drifted 2026-09-13 and turned every acceptance job red. The two Hermes constants move independently. A version review moves `HERMES_COMMIT` to the new release. `HERMES_INSTALL_COMMIT` moves only when that release needs a newer installer or the reviewed script breaks; re-verify the SHA256 when it moves. At the next Hermes version review, consider fetching `install.sh` from `HERMES_COMMIT` and deleting `HERMES_INSTALL_COMMIT`, so one commit identifies both. `claude.ai/install.sh` remains floating and hash-pinned; on its first drift, apply the same treatment if the vendor publishes a versioned URL.
- OMP installer (resolved 2026-09-23): `omp.sh/install` calls the unauthenticated GitHub REST API both with and without `--ref`; shared CI runner IPs can exhaust its rate limit. The installer now bypasses that script, resolves `github.com/can1357/oh-my-pi/releases/latest`, and verifies the platform asset against the resolved release's `SHA256SUMS.txt` before replacement. Download or integrity failure leaves an existing executable untouched; there is no older-release fallback. Clean acceptance keeps credentials isolated and exercises forced updates on macOS as well as Linux. Local Apple Silicon installation and forced update of `18.2.11` succeeded; clean-host matrix results remain separate CI evidence.
