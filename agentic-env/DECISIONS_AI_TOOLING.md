...
- MCP: `mcpServers: {codebase-memory-mcp}` gated by `disabledServers: [agentmemory, node_repl, codebase-memory-mcp, lean-ctx]` — wired but off, enable per session when the litmus passes. Gate installer-enforced on both OMP roots (`OMP_GATED_SERVERS` in `configure_agent_mcps.py`; bak `mcp.json.bak-leanctx-drop`, `.bak-gate`)
