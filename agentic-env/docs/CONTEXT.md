...
- **Primary harness**
  - The agent the stack is chosen for and validated against: `omp`. Its wiring is mandatory: the installer enforces it, the smoke test verifies it, and the stack doctor fails without it. Every other agent is a supported agent or an installed agent; neither drives tool selection.
  - _Avoid_: Default agent, main CLI, preferred agent

- **Supported agent**
  - An agent for which `agentic-env` manages the CLI, global skills, project-memory MCP configuration, and health checks. The user owns authentication and credentials. The supported agents are Hermes and OMP; only OMP's wiring is mandatory (see Primary harness).
  - _Avoid_: Partially supported agent, authenticated agent

- **Installed agent**
  - An agent for which `agentic-env` installs the CLI (latest release, verified against its reviewed version floor) and the curated skills, and nothing more. `agentic-env` writes and diagnoses no MCP configuration for it, and the doctor only warns when the binary is missing. The installed agents are Claude Code and Codex (measured need for managed wiring: zero; `DECISIONS_AI_TOOLING.md` Harness → "Agent tiers").
  - _Avoid_: Supported agent, secondary agent (unqualified)

- **Clean host**
  - A supported host provisioned from scratch, with no prior agent-stack installation or agent-specific configuration. A fresh install targets it, independent of any existing workstation's installed tools or configuration.
  - _Avoid_: Fresh machine (unqualified), blank VM

- **Native host acceptance**
  - Verifying clean provisioning on the target host's own operating system and kernel, with acceptance state isolated from any existing agent stack. Distinct from distro-container evidence, which exercises a container's userland on its host's kernel.
  - _Avoid_: Container acceptance, existing-stack health check

- **Machine provisioning**
  - The managed-stack operation that installs, configures, updates, and diagnoses the user-level agent stack. Distinct from standalone skill copying. Repository initialization, indexing, project-memory maintenance, uninstall, and rollback are outside provisioning. Provisioning diagnoses agent hook files but does not write them.
  - _Avoid_: Project onboarding, project lifecycle management

- **Project memory stack**
  - The project context layer agents use. On the primary harness, OMP core owns context I/O and search, Mnemopi holds narrative memory, and `codebase-memory-mcp` answers structural code-graph queries when its enable litmus fires. Hermes keeps `codebase-memory-mcp` and `agentmemory` wiring (unmeasured); installed agents get none.
...
- **Rejected**
  - A component evaluated and declined on graded evidence, including by its own pre-registered threshold. Reopening requires new external evidence.
...
- **Structural memory**
  - The indexed code graph that answers questions about code shape, symbols, routes, callers, dependencies, and impact. On the primary harness it is enabled on demand for cross-repo, whole-graph, or architecture questions, not mounted by default.
  - _Avoid_: Narrative memory, decision memory

- **Narrative memory**
  - The durable project history that preserves decisions, rationale, domain discoveries, and working context across agent sessions.
...
- **Managed MCP entry**
  - A user-level MCP entry whose expected definition `agentic-env` tracks. Missing entries are added; mismatched existing entries are reported for manual correction, never overwritten. Explicit primary-harness exclusion and gating policies still apply. Malformed configuration is rejected unmodified.
  - _Avoid_: Automatically repaired entry, any MCP entry

- **Read-interception policy**
  - A hook or instruction block that gates or redirects an agent's file reads and searches toward a query-first tool. Scoped per supported agent and counted across every configuration file that agent loads, not only the files in its own directory. At most one may be active per agent.
...
- **Fresh install**
  ...
  - _Avoid_: Deployment (unqualified; it can mean either fresh install or curated stack update)

- **Curated stack update**
  - A non-interactive maintenance run that reinstalls every installed agent-stack component. Floating components move to the current latest release; fixed-identity components stay at the reviewed artifact. The run fails if any result is below its reviewed version floor.
...
- **Reviewed version floor**
  - The minimum acceptable installed version of a stack binary, declared in `stack_metadata.STACK_VERSION_FLOORS` and set to the latest stable release at review time. A version at or above the floor is compliant; one below it fails install and the doctor. Distinct from _Compatibility floor_, which covers host prerequisites, not stack components.
  - _Avoid_: Version pin, exact-version match, tolerated drift

- **Fixed-identity artifact**
  - A stack component fetched by an identity that cannot float, because its fetch path is untrustworthy or its assets must be verified byte-for-byte. The identity is an installer commit (Hermes) or a per-architecture SHA256-pinned release archive (`codebase-memory-mcp`). It changes only when this repo's reviewed metadata changes. Independent of the version floor, which every component has.
  - _Avoid_: Pin (unqualified; ambiguous between the fetched artifact and the version floor)

- **Provisioner upgrade**
  - Replacing the installed `agentic-env` tool through `uv`. It stays separate from a curated stack update because `uv` owns the provisioner's installation source and lifecycle.
...
- **Stack doctor**
  - Read-only diagnosis of the primary-harness wiring contract. OMP failures are mandatory; Hermes wiring and secondary-agent binaries only warn. It runs at the end of bootstrap/update, exits non-zero only for mandatory failures, and gives corrective guidance without repairing configurations.
  - _Avoid_: Smoke test, automatic repair

- **Supported host**
  - An OS/architecture target with successful clean-host provisioning evidence; a compatible release asset is not enough. The intended targets are Apple Silicon macOS and Arch Linux x86_64. Direct CachyOS verification is deferred, and Intel macOS is outside acceptance scope. Windows skill-copy evidence does not establish a supported provisioning host.
...
Skill-specific states, distinct from the component lifecycle states above. A component is adopted when wired on by default; a skill is adopted on fit.

- **Installed (skill)**
  - Present in some skill store on this machine. It says nothing about whether the skill can fire or ever has.
  - _Avoid_: Available, active

- **Wired (skill)**
  - Has a mechanical trigger (forced injection, hook, or model-invocable frontmatter) that fires without relying on the user to invoke it.
  - _Avoid_: Enabled, configured

- **Adopted (skill)**
  - Kept in the curated skill default because it fits the user's workflows. The user judges fit; a usage-count threshold never decides it. Usage audits are diagnostic and only flag stale or duplicate skills for re-review.
  - _Avoid_: Proven, validated

- **Core (skill)**
  - An adopted skill named in a recipe's default step for its job. Whether the model or the user invokes it is a separate property, _Wired_.
  - _Avoid_: Default, primary, spine

- **Escalation (skill)**
  - An adopted skill outside every recipe's default step. It runs only when its named trigger fires and produces a stated work product. Distinct from _Gated_, which is wired but off; an escalation skill is on but not routed.
...
- **Skill store**
  - `~/.agents/skills`, the canonical copy of every installed skill. `skills add` writes here and symlinks `~/.claude/skills/<name>` and `~/.hermes/skills/<name>` into it; Codex reads it directly. OMP mounts it through `~/.claude/skills` (`enableClaudeUser`), never as its own root (`enableAgentsUser: false`), so each skill loads once.
  - _Avoid_: Recovery store, backup, archive

- **Vendored pack**
  - A skill pack whose installed source is a copy kept in this repository, carrying the upstream commit it was taken from and the upstream licence. The copy is the pin, not an upstream ref; a bump is a deliberate re-copy. An upstream-pinned pack, by contrast, installs from the author's repository at a tag.
  - _Avoid_: Fork, mirror, local pack

- **Recipe**
  - The per-job routing of skills for one kind of development task: which core skill runs, which escalations exist, and what triggers them. It is routing guidance, never a mandatory command chain.
...
- **Model-invoked**
  - A skill the model loads on its own judgment (registry `use_count`). It answers whether the model reaches for the skill, a different question from user-invoked.
  - _Avoid_: Registry use, load count

- **String-scan count**
  - Transcript matches on a skill name. It overstates use because it counts injected skill bodies, tool results, and discussion. Never a verdict input.
