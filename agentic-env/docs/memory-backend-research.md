...
**Stage 1b (2026-08-12): probes replayed on the now-populated Mnemopi bank** (same wording, same grading). Result: **P1 PASS** (drop + ADR-0008 + `mcp.json` state in top-4), **P2 PARTIAL** ("stays Mnemopi" at rank 3, c:0.4 — the one-owner rationale absent, rank 1 is the *challenger's* spec), **P3 PARTIAL** (applied 150K + read-out "kept" at rank 4; handoff/idle knobs absent), **P4 PARTIAL** (reflect runs with no external key — unlike the Hindsight zero-LLM arm — but answers only the lean-ctx half). **1 PASS + 3 PARTIAL vs 0/4 on the empty store → confirms the forensics: Stage 1's Mnemopi 0/4 measured retention, not recall.** Caveat: NOT corpus-controlled against Hindsight — the bank now contains explicit `retain` calls and bake-off meta-discussion (Hindsight's arm was raw pre-probe transcripts only); Stage 1b measures the realistic operating pipeline (auto + explicit retention), not a head-to-head. Net: populated-Mnemopi ≈ Hindsight@4,096 on decision recall, with opposite residual defects — Mnemopi's is **silent retention loss** (unfixable by tuning, needs a canary), Hindsight's is **recall budget** (fixable by config).

**Gap differential (2026-08-17, all suspects ruled out):** not OMP version (11 other project banks retained continuously across the same days/versions 17.1.5→17.3.5); not retain cadence (the 07-28 session had 28 user turns ≈ 7 due cycles, retained 0; 1-turn sessions retained fine pre-gap); not config (07-29 mid-gap backup shows `backend: mnemopi` correctly wired); not misrouting or bank drift (no dotfiles-marker rows in any of 276 banks in-window); not the repo hooks (born 07-28, after the 07-19/07-26 failures). Hard edges: works ≤07-14, dead 07-19→08-10, works ≥08-11 — with no memory-related change at either edge. **Proximate cause unknown; failure was project-local, silent, and self-healing.** That combination is undiagnosable after the fact and undetectable from inside — external retention canary is the only defense.
