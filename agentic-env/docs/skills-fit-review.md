...
Keep Matt Pocock as the engineering workflow spine. Use pstack for specific missing work products, not as a second operating mode. The strongest curated addition is `codebase-design`, which closes a conditional dependency of the existing `tdd` skill. The biggest pstack opportunity is reliable user-journey verification; the biggest installation risk is assuming that a present skill is self-contained and portable to OMP.

- **Core:** `grill-with-docs` → `wayfinder`/`implement` when needed → `code-review`; `grilling` and `domain-modeling` support the design conversation. Reach for `diagnosing-bugs`, `resolving-merge-conflicts`, `research`, and `tdd` by task, not by a compulsory slash-command ritual.
...
- **Skip a second style stack:** keep Ponytail/Caveman. Do not bulk-install pstack principles, an additional `tdd`/`teach`, or Cursor setup machinery into an OMP-first environment.
...
Coverage: **37 current Matt Pocock skills + 47 registered pstack skills = 84**, plus three dormant pstack automation bodies considered separately. The catalogues below link each skill to its inspected source.

These are fit judgments, not measured productivity gains. This review uses the documented work mix—product/backend/data work, issue/PR delivery, benchmarking, stack maintenance, and course preparation—without re-mining private project histories or treating August usage figures as current measurements. The previous [Pocock](pocock-skills-fit-report.html) and [pstack](pstack-skills-audit.html) audits have adjudication addenda; their uncorrected counts and superseded “install everything in Tier 1” conclusions are not adopted here.

## What is actually installed

Method: parse the checkout's [skill-pack manifest](../agentic_env/skill-packs.json), then inspect each named skill directory for a real `SKILL.md`, resolve symlinks, inspect invocation frontmatter, and compare the curated Claude-root copies with the skill store. Counts below are top-level valid skill directories, not usage counts, unique skills summed across agents, or a runtime registry dump. Nested Hermes bundles are outside these counts.
...
The default manifest is exactly **25 = Matt 13 + Ponytail 6 + Caveman 2 + pstack 4**. All are present in the store, Claude root, and Hermes root. This is a required roster, not an exact inventory ceiling: [the doctor's skill check](../agentic_env/stack_doctor.py) checks required files and the explicit lean-ctx exclusion; it does not reject every extra skill or prove all skill dependencies resolve.

**Extra skills in the Claude root:** `codebase-design`, `codebase-memory`, `graphify`, `improve-codebase-architecture`, `to-spec`, `to-tickets`, `wait-what`, `wizard`. Thus “pruned from the default manifest” does not mean “removed from this workstation.”

**Additional store-only extras relative to that root:** `diagnose`, `grill-me`, `loop-me`, `triage`, `write-a-skill`, `writing-great-skills`, `zoom-out`. The store also has old named entries without a `SKILL.md`; a directory name alone was not counted as an installed skill.

### Discovery and ownership caveats

1. `~/.omp/agent/config.yml` explicitly has `enableClaudeUser: true` and `enableAgentsUser: false`. The store is not mounted directly as an OMP root.
2. Nine curated Claude-root skills are **real directories, not symlinks**: `code-review`, `diagnosing-bugs`, `domain-modeling`, `grill-with-docs`, `grilling`, `implement`, `resolving-merge-conflicts`, `tdd`, `wayfinder`. Their `SKILL.md` contents currently match the store. That establishes body equality now, not shared ownership or future convergence. The overview's all-symlink description is the intended model, not an exact account of this machine.
3. **All four pstack picks** contain `disable-model-invocation: true`. The same flag appears on curated `grill-with-docs`, `wayfinder`, `implement`, `teach`, and `handoff`. They are manual entry points; installation alone is not automatic routing. A composed workflow may still explicitly request another skill. The flag is evidence about the authored invocation contract, not proof of successful execution in every harness.
4. The local skills lock records source, path, folder hash, and timestamps, but no source commit/ref for the inspected entries. A pinned installer manifest is not proof that every live body matches that pin. Source comparisons below distinguish installed text, the pin, and current upstream.
5. `graphify` is an extra, despite being described as on-demand in DECISIONS; it is not one of the four managed packs. No graph build, server activation, or catalogue-wide installation was needed for this review.
...
The checkout pins OMP `18.1.14`; the installed binary reports `18.1.19`. The doctor treats that mismatch as a mandatory failure, although the overview says versions ahead of pins merely warn. Secondary warnings also covered Hermes/Claude/Codex versions and Hermes's agentmemory arguments. These findings are recorded, not repaired: the skill-presence checks passed, but the whole-stack doctor did **not** pass. This is an installation/source review, not an end-to-end execution test of all skills or a new usage benchmark.
...
3. **Portability:** are its required skills, paths, tools, models, scripts, and external services actually available? “Good idea” and “runs as written” are different judgments.

The labels **core**, **situational**, and **skip as a default** below are recommendations, not replacements for the glossary's Installed/Wired/Adopted skill states. No new ADR or glossary change is warranted until a decision is made.
...
- Do not silently edit shared skill-store files during a retrospective. A Claude-root symlink can make that edit global across agents; a real copy can make it diverge instead.
- Do not treat “upstream has no tags” as an inability to pin. A reviewed commit can be pinned; deciding and verifying that change is separate from this source snapshot.
...
All 13 curated installed bodies match inspected current main. Only `implement` also matches the pinned body; the other twelve differ. This verifies text identity, not installation provenance. The important high-fit reference assets are present/current; local `triage` and `loop-me` retain older interview instructions. Local `writing-great-skills` is an obsolete predecessor, not an alias for current `writing-for-agents`. [Changelog][mp-changelog].

### Strongest addition: codebase-design

Recommend adding it to the curated set when implementation is authorized. This completes an existing composition: `tdd` calls it when interface/seam design is needed, but the manifest does not request it. Local installation masks that omission. Its value is deciding where complexity and observable tests belong—not another generic simplicity slogan. Use it when a change crosses too many callers or a regression has no sensible public seam. Its supporting deepening guide prefers replacing shallow tests once higher-level behavior is covered, rather than merely adding another layer. OMP still supplies navigation and parallel design execution. [TDD][mp-tdd], [design][mp-codebase-design], [deepening][mp-deepening].

### Important limits of the core composition

- **Grill-with-docs does not itself publish specs or tickets.** It composes an interview with domain documentation. **Wayfinder is a decision map**, not an implementation scheduler: research/prototype/grilling/task tickets resolve uncertainty. Upstream separately offers `to-spec` and `to-tickets`. Keeping those out of the local default is a workflow choice, not proof the contracts are identical. Reopen them if a persistent, independently executable spec or task graph is actually wanted. [Wrapper][mp-grill-with-docs], [wayfinder][mp-wayfinder], [router][mp-ask-matt].
- **Review scope can miss WIP.** `implement` requests review before committing, while `code-review` prescribes `git diff <fixed-point>...HEAD`. That excludes uncommitted/index/untracked changes. Preserve Standards + Spec review, but explicitly include the actual WIP when requested. This is a static source-contract finding, not a reproduced review failure. [Implement][mp-implement], [review][mp-code-review].
- **Tracker setup is a precondition, not a reason to install everything.** `code-review` hard-codes `docs/agents/issue-tracker.md`; wayfinder also needs tracker conventions. The uncurated setup skill supplies GitHub/GitLab/local templates, but existing repository configuration may already satisfy the requirement. Inspect that before running a writer. [Setup][mp-setup-matt-pocock-skills], [GitLab template][mp-gitlab-template].
- **Safety overrides blanket instructions.** The merge skill says to stage everything and never abort. Do not include unrelated work or override a user-requested abort. Diagnosis's feedback loop should trust user-established failures rather than repeatedly reconfirm them. [Merge][mp-resolving-merge-conflicts], [diagnosis][mp-diagnosing-bugs].
- **Literal tool names need translation.** Current compositions say “Call the Skill tool”; this OMP surface uses `read skill://…`. That is a portability requirement, not evidence the workflow failed here. [Invocation contract][mp-invocation].

### Useful outside the daily path

- **improve-codebase-architecture:** target a repeatedly painful subsystem. Ponytail asks what can be deleted; this skill asks which boundaries would make changes and tests local. Not a scheduled beautification pass. Its HTML scaffold loads third-party CDN scripts, so it is not an offline artifact despite self-contained wording. [Survey][mp-improve-codebase-architecture], [HTML asset][mp-architecture-html].
- **writing-for-agents:** a better candidate for deliberate prompt/skill-authoring work than retaining obsolete `writing-great-skills`. It addresses triggers, pointers, completion criteria, and environment facts. It still is not an automatic doc-drift detector; that prior rejection remains intact. [Skill][mp-writing-for-agents].
- **wizard / triage / to-questionnaire:** respectively human-only setup, incoming requests needing clarification, and asynchronous questions to a stakeholder. Distinct jobs; keep situational. Wizard's template is GitHub-CI-shaped, and triage can write comments/close issues, so neither is harmless read-only advice. [Wizard][mp-wizard], [template][mp-wizard-template], [triage][mp-triage], [questionnaire][mp-to-questionnaire].
- **teach / prototype:** pedagogical workspace and experimental design evidence. Matt's teach primarily teaches the user; it is not an arbitrary course generator. Prototype's browser-centered branches do not automatically cover native mobile UI. [Teach][mp-teach], [prototype][mp-prototype].
...
- **retro:** public beta exists, but the bucket README explicitly says “STUB: design notes only, not functional yet.” Its body proposes broader environment improvements, requires absent `writing-for-agents`, and lacks reflect's approved-patch loop. Keep on the watchlist, not a replacement for `reflect`. [Retro][mp-retro], [beta status][mp-beta].

### Complete Matt catalogue

**Legend:** U = upstream user-only (`disable-model-invocation: true`); M = model or user. E/P = promoted engineering/productivity; B = beta; X = misc. **C** = one of the current 13 curated names. “Recommended” means preserve/propose for Mihai's fit decision, not auto-adopt or run every time. “Skip” means skip from the default, not delete the installed copy.
...
**Snapshot:** `5bf2b1544db739998121a306340631963c2ff3de` (September 13), plugin metadata **0.15.2**, **47 registered skills**, including 23 principles. The prior audit covered 44. The three additional names are `make-bot-ui`, `principle-attack-the-premise`, and `principle-test-behavior-not-implementation`. [Manifest][ps-manifest]. The tags API still returns no tags; this is not a technical obstacle to commit pinning.

The installed verification generator and its three examples match current upstream byte-for-byte. `show-me-your-work`, `reflect`, and `recall` bodies differ; show's helper/template are unchanged, while reflect's four reviewer references differ. Much of the change is wording/model defaults; approval and transcript-boundary concerns already exist locally. No installed commit can be inferred from these comparisons.
...
**Verification is not supplied by installing the generator.** A browser driver is not a native mobile driver. The generated seed map is not proof all features were exercised. First create and run one relevant project recipe; preserve evidence through cleanup. [Feature-map example][ps-feature-map].

**Decision trail is not sealed proof.** The TSV helper does not capture artifacts, hash them to a revision, ensure freshness, publish them, or consume the verification-recorder table. It can point to existing proof but cannot replace its provenance mechanism. Its helper has no multi-writer initialization protocol; [INFERENCE] concurrent first writers can race on header creation. One coordinator is the simple safe owner. [Helper][ps-log-helper].

**Reflection is manual and has two write boundaries.** Approved skill changes and automatically filed tracker backlog are different actions. Recommend local proposals first; no public transcript-derived ticket or global skill edit without the selected scope. Read-only reviewers and transcript-as-untrusted protections are valuable. The existing digest fallback makes OMP adaptation more tractable than blindly copying a Cursor transcript path. [Reflect][ps-reflect], [synthesizer][ps-synthesizer].

**Recall is not self-contained.** Its technical-topic path requires `why`; it also names `unslop` and routes some requests to other pstack workflows. Show/reflect/technical-writing likewise reference siblings or Cursor authoring facilities. This is not a reason to install the entire dependency web. Either preserve a selected contract with explicit supported equivalents, or deliberately borrow a narrower method into an existing owner. Do not label a silently reduced workflow “vanilla pstack.”
...
2. **[technical-writing][ps-technical-writing]** — strong for runbooks, explanations, reference and tutorials. Distinguishing document purposes adds something Caveman's brevity does not. Keep repository style, explicit uncertainty and Spanish-language needs above its personal English/punctuation prescriptions. It references `unslop`; importing the whole style stack is unnecessary.
3. **[why][ps-why]** — historical intent from commits, issues, docs and other available records, with Direct/Supported/Inferred/Speculative/Unknown confidence. Code describes what happens, not automatically why its author chose it. Useful before removing a defensive path or changing a threshold. Unavailable integrations remain stated gaps; do not enable seven services merely because source categories exist. [Epistemics][ps-epistemics].
4. **[blast-radius][ps-blast-radius]** — reconsider the old “just fan-out” rejection. Its distinctive question is: **which one or two facts make this change safe, and can we execute code that proves them?** Wire formats, teardown order and dependency patches go beyond symbol references. Best for a tiny diff with wide effects. Native OMP supplies execution, not the safety-fact selection. This is a recommendation to reopen the lens, not an adopted policy change.
5. **[arena][ps-arena] / [interrogate][ps-interrogate]** — occasional high-stakes alternatives. Arena compares isolated candidates against one rubric, chooses a base and grafts coherent wins; it is not just a race. Interrogate gives independent model families the same intent/diff/rubric and returns judgments, not fixes. **It assumes the goal is correct, so it is not a replacement for grilling the plan.** Model diversity must be real routing, not different agent names.
6. **[automate-me][ps-automate-me]** — only when deliberately consolidating/exporting conventions. It mines histories and authors a personal routing profile, with commit/PR side effects. Existing Ponytail/Caveman/Mnemopi ownership and private transcript boundaries make it a poor routine default.

### Skip as installed defaults, retain the useful ideas

- **poteto-mode/setup-pstack:** not merely style text; a Cursor-oriented workflow/router/model configuration with unbundled tools and automation assumptions. It would introduce a second operating layer. **Swarm** has useful coverage/race/dropout semantics, but its default Cursor cloud workers are not native OMP fan-out. Do not silently upload local context to satisfy it. [Mode][ps-poteto-mode], [setup][ps-setup-pstack], [swarm][ps-swarm].
- **Style/principle cluster:** some useful distinctions—indirection versus mutable state, or isolate ownership before serializing—can inform the existing owner. That does not justify double injection. `no-comments` conflicts with load-bearing rationale/ceiling markers. Keep one `tdd` and one `teach` name/owner. [No-comments][ps-no-comments].
- **New attack-the-premise:** keep as a diagnostic idea when two fixes fail under the same assumption and worker/shard/actor imbalance may explain it. Not a universal answer to every repeated failure. [Source][ps-principle-attack-the-premise].
- **New test-behavior principle:** observable-behavior intent fits, but its blanket “still passes when the function returns undefined” classification includes assertions that would fail on undefined. Do not import that heuristic as an automatic test-deletion rule. [Source][ps-principle-test-behavior-not-implementation].
- **make-bot-ui:** Cursor/Grok Bot webhook dashboard, connector credentials, Tailscale and server exposure—not generic product UI work. Wrong default and substantial external side effects. [Source][ps-make-bot-ui].
- **Dormant Benny automation:** three auxiliary bodies (`setup-benny`, `triage-issue-reports`, `reproduce-and-fix-issues`) are outside the 47 registered skills. Slack/tracker/app-control/PR automation is their product, not a free OMP capability. Keep parked unless that exact integration is requested. [Automation README][ps-benny].

### Complete pstack catalogue

Compatibility labels: **Portable guidance** describes the method, not complete sibling dependency resolution. **Mostly portable** retains style/sibling assumptions. **Adapt** requires substantive path/tool/model/write-contract mapping. **Cursor-specific / not as written** is not a direct OMP workflow.

“Recommended” means semantic fit for Mihai to consider or retain, not installation/wiring/adoption. Every row links to the immutable inspected body.
...
The answers select the next work; they do not authorize global skill rewrites, tracker writes, or removing installed extras by implication. There is no need for a new ADR/glossary entry merely to record an audit recommendation.

## Source references

Skill links use exact inspected commits rather than moving `main`. Local installation facts come from the filesystem/lock/frontmatter inventory and read-only doctor run described above; historical fit context comes from the two adjudicated reports and operative DECISIONS document. Supporting scripts were inspected where relevant, not executed or subjected to a full security audit. No end-to-end OMP port or productivity benchmark is claimed.
