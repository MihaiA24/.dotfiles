...
## Decision
Copy the nine roster skill directories and `LICENSE` from `cursor/plugins` at commit `c1c0a32802223f4be824112dd83d33ad29a8b26c` (2026-09-14) into `agentic_env/vendored/pstack/`, with `UPSTREAM.md` recording commit, date, scope, and "local modifications: none". Manifest `source` becomes `./vendored/pstack/skills`; `_skill_pack_source` resolves a `./` source against the package directory and hands the absolute path to the CLI. The copy ships in the wheel (`pyproject.toml` artifacts), so `uv tool install` hosts install it without a checkout. The copy is the pin: a bump is a deliberate re-copy with an `UPSTREAM.md` update and a §6 Wiring note.
