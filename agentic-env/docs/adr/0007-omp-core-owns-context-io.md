...
- **The original version of this ADR was wrong in exactly the way this document warns about.** It generalised from one observation instead of measuring, and it would have been implemented had the workflow not been profiled first. The profile is now in `AI_CONTEXT_TOOLING_COMPARISON.md` under "Your workflow, measured".
