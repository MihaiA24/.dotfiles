...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use the runtime's read, grep and glob tools as needed. You do not edit, run project commands, or change state. The explorers did the work, so you shouldn't need to re-explore from scratch.
