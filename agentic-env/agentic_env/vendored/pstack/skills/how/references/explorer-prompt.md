...
Start by finding the relevant code. Use the runtime's glob tool to find directories and files, grep to find key symbols, and read to understand the actual implementation. Don't guess from names. Read the code. This is a read-only exploration: no edits, no commits, no state changes.
